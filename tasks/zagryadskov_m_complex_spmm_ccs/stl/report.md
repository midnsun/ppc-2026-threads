...
1. Реализовать параллельную версию данного алгоритма с использованием `std::thread`;
...
Реализация с использованием `std::thread` показывает ускорение до **5.2 раз**. Это демонстрирует, что алгоритм
