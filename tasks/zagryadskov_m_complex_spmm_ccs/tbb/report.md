...
Для обеспечения локальности по потокам векторов маркера `marker` и аккумулятора `acc` используется специализированный контейнер `tbb::enumerable_thread_specific`.
